...
partial class IntegrationsTests
{
  ...
  public async Task old_messages__resume_mongo_messages__old_messages_resumed ()
  {
    ...
    Message[] messages = [
      CreateMessage(CreateAddedToAgendaNotification("", ""), messageDate: currentDate.AddSeconds(-3), isPending: false),
    ...
    await ResumeMessagesMongoAsync(MongoDatabase, messageQueue, new FakeTimeProvider(currentDate), resumeMessagesOption, NullLoggerFactory.Instance.CreateLogger(""));
